...
const getAdminClient = () => createClient(process.env.SUPABASE_URL || '', process.env.SUPABASE_SERVICE_ROLE_KEY || '');
...
async function sendTelegramAlertInBackground(eventType, severity, message, userEmail, payload, source) {
    try {
        ...
        const { data, error: selectError } = await supabase
            ...
            .eq('id', ADMIN_UID)
            .maybeSingle();
        if (selectError || !data?.telegram_token?.trim() || !data?.telegram_chat_id?.trim())
            return;
 ...
 */
export async function logEvent(eventType, severity, message, userEmail, payload, source) {
    try {
