 ...
 */
export const PLAN_MONTHLY_SMS_LIMITS = {
    Starter: 150,
    Pro: 400,
    Power: 1400,
};
function normalizePlanToken(planName) {
 ...
 */
export function monthlySmsLimitForPlan(planName, explicit) {
    ...
    if (!p)
        return 150;
    if (p.includes('power'))
        return PLAN_MONTHLY_SMS_LIMITS.Power;
    if (p.includes('pro'))
        return PLAN_MONTHLY_SMS_LIMITS.Pro;
    if (p.includes('starter'))
        return PLAN_MONTHLY_SMS_LIMITS.Starter;
    const exact = Object.keys(PLAN_MONTHLY_SMS_LIMITS).find((k) => k.toLowerCase() === p);
    if (exact)
        return PLAN_MONTHLY_SMS_LIMITS[exact];
    return 150;
}
//# sourceMappingURL=subscriptionPlanLimits.js.map