...
import { isSupportedOnboardingCountryCode, slotCountryMatchesOnboardingIso, } from '../_helpers/slotCountryMapping.js';
...
function parseCentsSetting(content) {
    const n = typeof content === 'string' ? parseInt(content, 10) : Number(content);
    return Number.isFinite(n) ? n : 0;
}
async function getFinanceCostRatesCents() {
    try {
        ...
        const costPerSlotMonthCents = parseCentsSetting(slotRow?.data?.content);
        const costPerSmsCents = parseCentsSetting(smsRow?.data?.content);
...
async function persistSubscriptionInvoiceFromWebhook(params) {
    ...
    let nextBillingIso = periodEndIso;
    try {
    ...
    if (invUpsertErr) {
        console.error('[subscription_invoices] upsert failed (Supabase)', {
            ...
            hint: invUpsertErr.hint,
        });
        throw new Error(`[subscription_invoices] persist failed: ${invUpsertErr.message} (code=${invUpsertErr.code ?? 'n/a'})`);
    }
    return fullInv;
}
/** Columnas necesarias para invoice.payment_succeeded + ledger + email. */
const SUBSCRIPTION_ROW_FOR_INVOICE_PAYMENT = 'id, user_id, plan_name, status, slot_id, phone_number, monthly_limit, billing_type, currency, stripe_session_id, stripe_subscription_id, created_at';
function digitsOnly(s) {
    return s.replace(/\D/g, '');
}
/** UUID en metadata (userId / user_id). */
function parseUserIdFromMetadata(raw) {
...
function mergeMetadataRecord(target, meta) {
...
async function extractInvoiceLinkageHints(invoice, stripeSubId) {
    const hints = {};
 ...
 */
async function findSubscriptionRowByStripeSubscriptionId(stripeSubId) {
    const nonCanceled = () => supabaseAdmin
        .from('subscriptions')
    ...
    if (byStripeSub) {
        return { data: byStripeSub, error: null };
    }
    if (stripeSubId.startsWith('cs_')) {
        ...
        return { data: bySession ?? null, error: null };
    }
    return { data: null, error: null };
}
/** Defensa en profundidad: si el SELECT devolviera una fila cancelada, no usarla para billing. */
function discardCanceledSubscriptionRowForInvoicePayment(row, reason, stripeSubId) {
    if (!row)
        return null;
    if (!isCanceledSubscriptionStatus(String(row.status ?? '')))
        return row;
 ...
 */
function pickLiveSubscriptionCandidate(rows, stripeSubId) {
    ...
    const repair = live.find((r) => r.stripe_subscription_id == null ||
...
async function listSubscriptionRowsByUserAndSlot(userId, slotId, limit = FALLBACK_CANDIDATE_LIMIT) {
...
async function listSubscriptionRowsByUserAndPhone(userId, phoneHint, limit = PHONE_FALLBACK_ROW_CAP) {
    ...
    if (byVariants?.length) {
        return { data: byVariants, error: null };
    }
...
async function triggerEmail(event, userId, data) {
    ...
    try {
        let email = data?.to_email ?? data?.email ?? data?.to ?? undefined;
        if (!email) {
...
async function createNotification(userId, title, message, type, sourceStripeEventId, sourceNotificationKey) {
    try {
        ...
        void logEvent('IN_APP_NOTIFICATION_DISPATCHED', 'info', `${title}`, null, {
            user_id: userId,
            type,
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const markWebhookFailed = async (message, stack) => {
            if (!stripeEventId)
                return;
            try {
                await supabaseAdmin
        ...
        if (event.type === 'checkout.session.completed') {
            ...
            let sessionMeta = rawMeta && typeof rawMeta === 'object' ? { ...rawMeta } : {};
            if (!sessionMeta.upgrade && session.subscription) {
                try {
                    const subscription = await stripe.subscriptions.retrieve(session.subscription);
                    sessionMeta = { ...sessionMeta, ...subscription.metadata };
                }
                catch (e) {
            ...
            if (sessionMeta.upgrade === 'true' && sessionMeta.slot_id && sessionMeta.user_id) {
                ...
                if (existingLiveForSlot) {
                    ...
                    if (upInsErr) {
                        ...
                        console.log('[PURCHASE] subscription upsert OK', {
                            subscriptionId: upInsRow?.id ?? null,
                ...
                const { data: userData } = await supabaseAdmin
                    ...
                    .maybeSingle();
                if (userData?.email) {
                    await triggerEmail('subscription_activated', upgradeMeta.user_id, {
                        plan_name: upgradeMeta.new_plan_name,
                ...
                try {
                    ...
                    await sendTelegramNotification('upgrade_success', upgradeMeta.user_id, {
                        phone: phoneNumber,
            ...
            const phoneFromMeta = (sessionMeta.phoneNumber ?? sessionMeta.phone_number ?? '');
            /** Código ISO país onboarding (metadata.region); mismo criterio que checkout + slots.country */
            const onboardingCountryIso = String(sessionMeta.region ?? sessionMeta.regionCode ?? '')
                .trim()
                .toUpperCase();
            console.log('[WEBHOOK] metadata:', JSON.stringify(session.metadata));
            console.log(`[WEBHOOK INIT] sessionId: ${session.id}, userId: ${userId}, slotId: ${slotId}, planName: ${planName}, isAnnual(meta): ${isAnnual}, phoneFromMeta: ${phoneFromMeta || '(vacío)'}`);
            if (!userId || !slotId) {
            ...
            try {
                ...
                const { data: slot, error: slotError } = await supabaseAdmin
                    .from('slots')
                    .select('phone_number, country, status, reservation_token, reservation_expires_at, reservation_user_id, reservation_stripe_session_id')
                    .eq('slot_id', slotId)
                    .maybeSingle();
                if (slotError) {
                ...
                if (enforceReservationValidation) {
                    const countryOk = !onboardingCountryIso || slotCountryMatchesOnboardingIso(slot?.country, onboardingCountryIso);
                    ...
                    reservationValid = !!slot &&
                        ...
                        new Date(slot.reservation_expires_at).getTime() > nowForReservationMs &&
                    ...
                    if (slot?.status === 'reserved' && slot?.reservation_token === reservationTokenFromMeta) {
                        const expiresAtMs = slot.reservation_expires_at
                            ? new Date(slot.reservation_expires_at).getTime()
                            : 0;
                ...
                const existingActivationState = exists?.activation_state ?? null;
                if (!exists) {
                    const correctAmount = amount;
                    console.log(`[WEBHOOK DEBUG] planName: ${planName}, isAnnual(meta): ${isAnnual}, billingType: ${billingType}, amount(unit_amount): ${amount}, correctAmount: ${correctAmount}`);
                    ...
                    subscriptionId = insertedData?.id ?? subscriptionId;
                    console.log('[PURCHASE] subscription upsert OK', {
                        subscriptionId,
                ...
                if (reservationValid && (!exists || existingActivationState === 'paid')) {
                    await createNotification(userId, '✅ Pago confirmado', 'Tu pago fue confirmado. Estamos activando tu SIM.', 'success', session.id, 'activation_paid');
                }
                ...
                if (reservationValid) {
                    ...
                    if (activeSubId) {
                        ...
                        if (nextBillingDateIso)
                            payload.next_billing_date = nextBillingDateIso;
                    ...
                    if (existingActivationState !== 'provisioned' && existingActivationState !== 'on_air') {
                        await createNotification(userId, '📦 SIM aprovisionada', 'Tu número fue asignado en infraestructura. Cerrando activación operativa...', 'activation', session.id, 'activation_provisioned');
                    }
                    ...
                    if (existingActivationState !== 'on_air') {
                        await createNotification(userId, '🚀 ¡Línea activada!', trialMsg, 'activation', session.id, 'activation_on_air');
                    }
                    if (activeSubId) {
                        const payload = {
                            activation_state: 'on_air',
                    ...
                    if (existingActivationState !== 'failed' && enforceReservationValidation) {
                        await createNotification(userId, '⚠️ Activación fallida', 'No pudimos activar tu SIM porque la reserva del slot no fue válida. Intenta nuevamente o revisa tu panel.', 'error', session.id, 'activation_failed');
                    }
                }
            }
            catch (err) {
            ...
            if (!phoneForEmail && slotId) {
                try {
                    ...
                    phoneForEmail = slotRow?.phone_number ?? '';
                }
                catch (e) {
            ...
            if (activationSucceeded) {
                try {
                    void logEvent('EMAIL_DISPATCHED', 'info', 'purchase_success', null, {
                        user_id: userId,
                ...
                try {
                    ...
                    void logEvent('TELEGRAM_DISPATCHED', 'info', 'new_purchase', null, {
                        user_id: userId,
            ...
            const cancelAtPeriodEndChanged = previousAttributes.cancel_at_period_end === false &&
            ...
            try {
                const { data: primaryCandidates, error: primaryErr } = await supabaseAdmin
                    ...
                    .limit(5);
                if (primaryErr) {
                    await logEvent('cancel_failed_no_live_subscription', 'error', primaryErr.message, undefined, { phase: 'customer.subscription.updated', stripeSubId, error_code: primaryErr.code }, 'stripe');
                ...
                await logEvent('cancel_lookup_primary', 'info', 'cancel: lookup primario customer.subscription.updated', undefined, { phase: 'customer.subscription.updated', stripeSubId, found_candidates: (primaryCandidates ?? []).length }, 'stripe');
                let sub = null;
                if (!primaryCandidates || primaryCandidates.length === 0) {
                    ...
                    await logEvent('cancel_failed_no_live_subscription', 'error', 'cancel: no se encontró fila local para customer.subscription.updated (solo stripe_subscription_id)', undefined, { phase: 'customer.subscription.updated', stripeSubId }, 'stripe');
                    await markWebhookProcessed();
                    return res.status(200).json({
                        received: true,
                        ignored: true,
                ...
                if (primaryCandidates.length > 1) {
                    await logEvent('cancel_failed_multiple_live_candidates', 'error', 'cancel: múltiples suscripciones locales candidatas por stripe_subscription_id', undefined, { phase: 'customer.subscription.updated', stripeSubId, candidate_ids: primaryCandidates.map((r) => r.id) }, 'stripe');
                ...
                if (existingLocalStatus === 'canceled' || existingLocalStatus === 'cancelled') {
                    await logEvent('cancel_local_subscription_updated', 'info', 'cancel: customer.subscription.updated ignorado (fila local ya cancelada)', undefined, { phase: 'customer.subscription.updated', stripeSubId, local_subscription_id: sub.id }, 'stripe');
                    await markWebhookProcessed();
                ...
                if (existingLocalStatus === 'canceled' || existingLocalStatus === 'cancelled')
                    nextLocalStatus = 'canceled';
                const statusUpdatePayload = { status: nextLocalStatus };
                ...
                if (sub.slot_id &&
                    ...
                    const { data: slotRowPre, error: slotPreErr } = await supabaseAdmin
                        ...
                        .maybeSingle();
                    if (slotPreErr) {
                        await logEvent('cancel_failed_no_live_subscription', 'error', 'cancel: fallo consultando slots (pre-RPC updated)', undefined, {
                            phase: 'customer.subscription.updated',
                            stripeSubId,
                            slot_id: sub.slot_id,
                        ...
                        const shouldReleasePre = slotStatusPre !== 'libre' &&
                        ...
                        if (shouldReleasePre) {
                            ...
                            if (rpcReleaseErr) {
                                await logEvent('cancel_failed_no_live_subscription', 'error', rpcReleaseErr.message, undefined, {
                                    phase: 'customer.subscription.updated',
                                    stripeSubId,
                                    slot_id: sub.slot_id,
                            ...
                            await logEvent('cancel_slot_released', 'info', 'cancel: slot liberado tras customer.subscription.updated (release_slot_atomic)', undefined, { phase: 'customer.subscription.updated', stripeSubId, local_subscription_id: sub.id, slot_id: sub.slot_id }, 'stripe');
                        }
                    }
                }
                ...
                if (nextLocalStatus === 'canceled' && existingLocalStatus !== 'canceled' && existingLocalStatus !== 'cancelled') {
                    await logEvent('cancel_local_subscription_updated', 'info', 'cancel: subscriptions local status actualizado a canceled', undefined, { phase: 'customer.subscription.updated', stripeSubId, local_subscription_id: sub.id, slot_id: sub.slot_id }, 'stripe');
                }
                ...
                const localIsCanceled = nextLocalStatus === 'canceled' || nextLocalStatus === 'cancelled' || existingLocalStatus === 'canceled' || existingLocalStatus === 'cancelled';
                ...
                if (prevStatus === 'trialing' && newStatus === 'active') {
                    await createNotification(sub.user_id, '✅ Trial completado — Plan activo', `Tu plan ${sub.plan_name} está ahora activo. Gracias por continuar con Telsim.`, 'subscription');
                }
                ...
                if (isCanceledNow || isCanceledAtPeriodEnd) {
                    ...
                    if (!slotReleased && sub.slot_id) {
                        ...
                        if (!slotErrAfter && slotRowAfter) {
                            ...
                            await logEvent('cancel_slot_release_skipped', 'info', 'cancel: liberación vía RPC no aplicada; estado slot tras updated', undefined, {
                                phase: 'customer.subscription.updated',
                                stripeSubId,
                                slot_id: sub.slot_id,
                    ...
                    await createNotification(sub.user_id, '⚠️ Suscripción cancelada', slotReleased
                        ? `Tu plan ${sub.plan_name} fue cancelado. Tu número ha sido liberado. Puedes activar un nuevo plan cuando quieras.`
                        : `Tu plan ${sub.plan_name} fue cancelado. (El slot asociado pudo no haberse liberado por consistencia operativa).`, 'warning');
            ...
            try {
                ...
                if (sub?.user_id) {
                    void logEvent('SUBSCRIPTION_CREATED_WEBHOOK_IGNORED', 'info', 'subscription.created no envía purchase_success; se espera checkout.session.completed', null, {
                        user_id: sub.user_id,
            ...
            try {
                ...
                try {
                    ...
                    await supabaseAdmin.from('finance_events').upsert({
                        ...
                        currency: (invoice.currency ?? sub.currency ?? 'usd'),
                        amount_cents: null,
                ...
                await supabaseAdmin
                    ...
                    .eq('id', sub.id)
                    .in('status', LIVE_STATUSES);
                await createNotification(sub.user_id, '🔴 Pago fallido — Acción requerida', `No pudimos cobrar tu plan ${sub.plan_name}. Actualiza tu método de pago en Billing para no perder el acceso.`, 'error', stripeEventId, 'payment_failed');
                void logEvent('EMAIL_DISPATCHED', 'info', 'invoice_failed', null, {
                    user_id: sub.user_id,
                ...
                void logEvent('TELEGRAM_DISPATCHED', 'info', 'invoice_failed', null, {
                    user_id: sub.user_id,
            ...
            const { data: sub, error: subErr } = await supabaseAdmin
                ...
                .maybeSingle();
            if (subErr) {
                await logEvent('WEBHOOK_ERROR', 'error', subErr.message, invoice.customer_email ?? undefined, { context: event.type, stripe_subscription_id: stripeSubId, error_code: subErr.code }, 'stripe');
            ...
            if (!sub) {
                ...
                void logEvent('invoice_ignored_no_live_subscription', 'info', `invoice webhook ignorado: no existe fila local viva por stripe_subscription_id`, undefined, { context: event.type, stripe_subscription_id: stripeSubId, invoice_id: invoice.id }, 'stripe');
                await markWebhookProcessed();
                return res.status(200).json({ received: true });
            }
            const subTyped = sub;
            ...
            try {
                ...
                await supabaseAdmin.from('finance_events').upsert({
                    ...
                    currency: (invoice.currency ?? subTyped.currency ?? 'usd'),
            ...
            if (subTyped.status === 'past_due') {
                ...
                if (pastDueErr)
                    throw pastDueErr;
                await createNotification(subTyped.user_id, '✅ Pago procesado', `El pago de tu plan ${subTyped.plan_name} fue exitoso. Tu servicio continúa activo.`, 'success');
            }
        ...
        // Fase 5: refunds / chargebacks (modelo preparado, sin notificaciones)
            ...
            try {
                ...
                const stripeSubscriptionId = (meta.stripe_subscription_id ?? meta.stripeSubscriptionId ?? meta.subscription_id ?? meta.subscriptionId ?? '');
                const stripeSessionId = (meta.stripe_session_id ?? meta.stripeSessionId ?? '');
                ...
                const currency = (refund.currency ?? subRow?.currency ?? 'usd');
                await supabaseAdmin.from('finance_events').upsert({
                    stripe_event_id: stripeEventId,
                    stripe_event_type: event.type,
            ...
            try {
                ...
                const currency = (charge.currency ?? subRow?.currency ?? 'usd');
                await supabaseAdmin.from('finance_events').upsert({
                    stripe_event_id: stripeEventId,
                    stripe_event_type: event.type,
                    finance_event_type: 'refund',
            ...
            try {
                ...
                const currency = (dispute.currency ?? subRow?.currency ?? 'usd');
                await supabaseAdmin.from('finance_events').upsert({
                    stripe_event_id: stripeEventId,
                    stripe_event_type: event.type,
            ...
            try {
                const { data: primaryCandidates, error: primaryErr } = await supabaseAdmin
                    ...
                    .order('created_at', { ascending: false })
                    .limit(5);
                await logEvent('cancel_lookup_primary', 'info', 'cancel: lookup primario customer.subscription.deleted', undefined, { phase: 'customer.subscription.deleted', stripeSubId: subId, found_candidates: (primaryCandidates ?? []).length }, 'stripe');
                if (primaryErr) {
                    await logEvent('cancel_failed_no_live_subscription', 'error', primaryErr.message, undefined, { phase: 'customer.subscription.deleted', stripeSubId: subId, error_code: primaryErr.code }, 'stripe');
                    await markWebhookFailed(primaryErr.message);
                ...
                if (!primaryCandidates || primaryCandidates.length === 0) {
                    ...
                    await logEvent('cancel_failed_no_live_subscription', 'error', 'cancel: customer.subscription.deleted sin fila local', undefined, { phase: 'customer.subscription.deleted', stripeSubId: subId }, 'stripe');
                    await markWebhookProcessed();
                    return res.status(200).json({
                        received: true,
                        ignored: true,
                ...
                if (primaryCandidates.length > 1) {
                    await logEvent('cancel_failed_multiple_live_candidates', 'error', 'cancel: múltiples suscripciones locales candidatas por stripe_subscription_id (deleted)', undefined, { phase: 'customer.subscription.deleted', stripeSubId: subId, candidate_ids: primaryCandidates.map((r) => r.id) }, 'stripe');
                    // Regla: en deleted NO rechazamos; actualizamos todas las filas del mismo stripe_subscription_id.
                }
                const sub = primaryCandidates[0];
                ...
                const primarySlotIds = Array.from(new Set((primaryCandidates ?? [])
                    .map((c) => c.slot_id)
                ...
                for (const slotIdToRelease of primarySlotIds) {
                    ...
                    if (rpcReleaseErr) {
                        await logEvent('cancel_failed_no_live_subscription', 'error', rpcReleaseErr.message, undefined, { phase: 'customer.subscription.deleted', stripeSubId: subId, slot_id: slotIdToRelease, error_code: rpcReleaseErr.code }, 'stripe');
                        await markWebhookFailed(rpcReleaseErr.message);
                        return res.status(500).json({ received: false, error: rpcReleaseErr.message, phase: 'customer.subscription.deleted' });
                    }
                    await logEvent('cancel_slot_released', 'info', 'cancel: slot liberado tras customer.subscription.deleted (RPC)', undefined, { phase: 'customer.subscription.deleted', stripeSubId: subId, local_subscription_id: sub.id, slot_id: slotIdToRelease }, 'stripe');
                }
                ...
                const { error: updateErr } = await supabaseAdmin
                    ...
                    .eq('stripe_subscription_id', subId);
                if (updateErr) {
                    await logEvent('cancel_failed_no_live_subscription', 'error', updateErr.message, undefined, { phase: 'customer.subscription.deleted', stripeSubId: subId, local_subscription_id: sub.id, error_code: updateErr.code }, 'stripe');
                ...
                await logEvent('cancel_local_subscription_updated', 'info', 'cancel: subscriptions local status actualizado a canceled (deleted)', undefined, {
                ...
                await createNotification(sub.user_id, '🔴 Suscripción terminada', `Tu plan ${sub.plan_name} fue cancelado definitivamente. Reactiva tu suscripción cuando quieras.`, 'error', stripeEventId, 'subscription_cancelled');
                ...
                if (userData?.email && userId) {
                    void logEvent('EMAIL_DISPATCHED', 'info', 'subscription_cancelled', null, { user_id: userId, template: 'template_email_subscription_cancelled', channel: 'email' }, 'stripe');
                ...
                void logEvent('TELEGRAM_DISPATCHED', 'info', 'cancellation', null, { user_id: userId, template: 'template_telegram_cancellation', channel: 'telegram' }, 'stripe');
        ...
        try {
            ...
            console.error('[WEBHOOK] logEvent falló (DB/logger):', logErr?.message);
            try {
                ...
                console.error('[WEBHOOK] audit_logs insert falló:', dbErr?.message);
            }
        }
