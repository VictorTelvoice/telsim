...
function buildPeriodKeysDay(startDateOnly, endDateOnly) {
...
function buildPeriodKeysMonth(startDateOnly, endDateOnly) {
...
function computeGrossPct(netCash, gross) {
    if (netCash === 0)
        return 0;
    ...
    try {
        ...
        if (isAdmin && scopedUserId == null) {
            ...
            const { data: rows, error } = await supabaseAdmin
                .from(view)
                .select('period_date,cash_revenue_cents,booked_sales_cents,booked_monthly_equivalent_cents,estimated_cost_cents,gross_margin_cents,gross_margin_pct')
                .gte('period_date', startDateOnly)
                .lte('period_date', endDateOnly)
                .order('period_date', { ascending: true });
            if (error)
                return res.status(500).json({ error: `trends view error: ${error.message}` });
        ...
        for (const ev of events ?? []) {
            ...
            if (granularity === 'day') {
                ...
                if (!buckets[key])
                    buckets[key] = { cash: 0, refunds: 0, chargebacks: 0, booked: 0, bookedMonthlyEq: 0, estimatedCost: 0 };
                if (ev.finance_event_type === 'cash_revenue')
                    buckets[key].cash += toNumber(ev.amount_cents);
                if (ev.finance_event_type === 'refund')
                    buckets[key].refunds += toNumber(ev.amount_cents);
                if (ev.finance_event_type === 'chargeback')
                    buckets[key].chargebacks += toNumber(ev.amount_cents);
                ...
                if (ev.finance_event_type === 'estimated_cost')
                    buckets[key].estimatedCost += toNumber(ev.amount_cents);
            }
            else {
                d.setUTCDate(1);
                const key = d.toISOString().slice(0, 10);
