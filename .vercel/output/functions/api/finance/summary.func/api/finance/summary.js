    ...
    try {
        ...
        if (scopedUserId == null) {
            ...
            const { data: ledgerRows, error: ledgerErr } = await supabaseAdmin
                ...
                .select('cash_revenue_cents, booked_sales_cents, booked_monthly_equivalent_cents, failed_payments_count, revenue_at_risk_cents, estimated_cost_cents, refunds_amount_cents, chargebacks_amount_cents, period_date')
            ...
            totals = (ledgerRows ?? []).reduce((acc, r) => {
        ...
        const gross_margin_pct = totals.net_cash_revenue_cents === 0
            ? 0
        ...
        if (scopedUserId == null) {
            ...
            const paidCount = activeSubs.filter((s) => s.activation_state === 'paid').length;
            const provisionedCount = activeSubs.filter((s) => s.activation_state === 'provisioned').length;
            const onAirCount = activeSubs.filter((s) => s.activation_state === 'on_air').length;
            const failedCount = activeSubs.filter((s) => s.activation_state === 'failed').length;
            let mrrCents = 0;
            for (const s of activeSubs) {
                if (s.status !== 'active')
                    continue; // trialing no cuenta como MRR
