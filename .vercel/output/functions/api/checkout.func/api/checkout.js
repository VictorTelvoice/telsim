...
import { applySlotCountryFilter, isSupportedOnboardingCountryCode, } from './_helpers/slotCountryMapping.js';
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const regionCode = typeof region === 'string' && region.trim().length > 0 ? region.trim().toUpperCase() : '';
        ...
        if (regionCode && !isSupportedOnboardingCountryCode(regionCode)) {
            ...
            return res.status(400).json({
                error: `El país "${regionCode}" no está configurado para asignación de línea. Actualiza el catálogo o contacta soporte.`,
        ...
        if (customerId && !forceManual) {
            try {
                ...
                if (defaultPaymentMethod) {
                    if (isUpgrade && slot_id) {
                        ...
                        if (activeSub && (activeSub.stripe_subscription_id || activeSub.stripe_session_id)) {
                            let stripeSubId = activeSub.stripe_subscription_id;
                            if (!stripeSubId) {
                                const sessionId = activeSub.stripe_session_id;
                    ...
                    if (!isUpgrade) {
                        let freeQ = supabaseAdmin
                            .from('slots')
                            .select('slot_id, phone_number')
                            .eq('status', 'libre')
                            .is('assigned_to', null);
                        if (regionCode)
                            freeQ = applySlotCountryFilter(freeQ, regionCode);
                        ...
                        if (freeSelectErr) {
                            console.error('[CHECKOUT][slots] one-click SELECT libre failed:', {
                                ...
                                hint: freeSelectErr.hint,
                            });
                            throw new CheckoutHttpError(500, 'SLOT_QUERY_FAILED', `Error al consultar inventario de líneas: ${freeSelectErr.message}`);
                        }
                        if (freeSlot) {
                            ...
                            if (oneClickOccupyErr) {
                                ...
                                throw new CheckoutHttpError(500, 'SLOT_OCCUPY_FAILED', `Error al ocupar la línea: ${oneClickOccupyErr.message}`);
                            }
                            ...
                            if (existingLiveSub) {
                                ...
                                throw new CheckoutHttpError(409, 'LIVE_SUBSCRIPTION_EXISTS_FOR_SLOT', 'No se insertó una nueva subscription viva porque el slot ya tiene otra viva.');
                            }
                ...
                if (oneClickErr instanceof CheckoutHttpError)
                    throw oneClickErr;
        ...
        let targetPhoneNumber = typeof phoneNumber === 'string' ? phoneNumber : '';
        ...
        const reserveSlotForCheckout = async () => {
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
                ...
                let expiredReservedSlot = null;
                if (!freeSlot) {
                    let expiredQ = supabaseAdmin
                        .from('slots')
                        .select('slot_id, phone_number')
                        .eq('status', 'reserved')
                        .is('assigned_to', null)
                    ...
                    if (regionCode)
                        expiredQ = applySlotCountryFilter(expiredQ, regionCode);
                    ...
                    if (expiredErr) {
                        ...
                        throw new CheckoutHttpError(500, 'SLOT_QUERY_FAILED', `Error al consultar reservas expiradas: ${expiredErr.message}`);
                    }
                    expiredReservedSlot = data;
                }
                const slotToReserve = freeSlot ?? expiredReservedSlot;
                const candidateStatus = freeSlot ? 'libre' : 'reserved';
                if (!slotToReserve) {
                    if (attempt === 2) {
                        // En este punto no encontramos slots candidatos en ninguna iteración.
                        throw new CheckoutHttpError(422, 'NO_SLOTS_AVAILABLE', 'No hay slots disponibles para reservar en este momento.');
                    }
                    continue;
                }
                ...
                if (reserveErr) {
                    ...
                    throw new CheckoutHttpError(500, 'SLOT_RESERVATION_UPDATE_FAILED', reserveErr.message || 'Error al reservar el slot.');
                }
                if (reservedSlot) {
                    return {
                        slotId: reservedSlot.slot_id,
                        phone: reservedSlot.phone_number ?? '',
            ...
            if (hadAnyCandidate) {
                throw new CheckoutHttpError(409, 'SLOT_RESERVATION_CONFLICT', 'Hubo un conflicto temporal al reservar tu slot. Intenta nuevamente.');
            }
        ...
        if (!isUpgrade) {
            ...
            targetPhoneNumber = slotRow?.phone_number ?? '';
        }
        const sessionConfig = {
        ...
        if (!isUpgrade) {
            ...
            const tokenToWrite = (meta.reservation_token || '');
            await supabaseAdmin
                .from('slots')
