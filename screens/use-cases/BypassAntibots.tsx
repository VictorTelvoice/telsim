  ...
  const steps = [
    {
      ...
      title: t('bypass.step1_title'),
      desc: t('bypass.step1_desc')
    },
    {
      icon: <Terminal className="size-6" />,
      title: t('bypass.step2_title'),
      desc: t('bypass.step2_desc')
    },
    {
      icon: <Zap className="size-6" />,
      title: t('bypass.step3_title'),
      desc: t('bypass.step3_desc')
    }
  ];
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark font-display pb-32">
      ...
      <div className="relative h-72 overflow-hidden bg-slate-900">
        ...
        <header className="relative z-20 flex items-center justify-between px-6 py-5">
          ...
          <span className="text-[10px] font-black text-white/50 uppercase tracking-[0.3em]">{t('bypass.automation_protocol')}</span>
        ...
        <div className="relative z-20 px-8 pt-6 flex flex-col items-center text-center">
          ...
          <h1 className="text-3xl font-black text-white leading-tight tracking-tight">
            {t('bypass.title').split(' ')[0]} <span className="text-blue-300">{t('bypass.title').split(' ')[1]}</span>
          </h1>
          <p className="text-white/60 text-xs font-bold uppercase tracking-[0.2em] mt-2">{t('bypass.real_sim_tech')}</p>
        </div>
      </div>
      ...
      <main className="px-6 -mt-8 relative z-30 space-y-8">
        ...
        <div className="bg-white dark:bg-surface-dark rounded-[2.5rem] p-6 shadow-soft border border-slate-100 dark:border-slate-800">
          <div className="flex gap-4 items-center mb-6">
            <div className="flex-1 flex flex-col items-center gap-2 p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 opacity-50">
                ...
                <span className="text-[9px] font-black uppercase text-slate-500 tracking-widest">{t('bypass.voip_blocked').split(' - ')[0]}</span>
                <span className="text-[10px] font-bold text-red-500">{t('bypass.voip_blocked').split(' - ')[1]}</span>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center gap-2 p-3 rounded-2xl bg-blue-50 dark:bg-blue-900/20 border border-primary/20 shadow-sm ring-2 ring-primary/10">
                ...
                <span className="text-[9px] font-black uppercase text-primary dark:text-blue-400 tracking-widest">{t('bypass.telsim_accepted').split(' - ')[0]}</span>
                <span className="text-[10px] font-bold text-emerald-500">{t('bypass.telsim_accepted').split(' - ')[1]}</span>
            </div>
          </div>
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium leading-relaxed text-center px-4">
            {t('bypass.expert_desc')}
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between px-2">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{t('bypass.implementation_flow')}</h3>
          </div>
          {steps.map((step, idx) => (
        ...
        <div className="flex flex-col items-center text-center gap-2 py-4 opacity-30">
            ...
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.4em]">{t('bypass.military_grade')}</p>
        </div>
      </main>
      ...
      <div className="fixed bottom-0 left-0 w-full p-6 bg-gradient-to-t from-background-light dark:from-background-dark via-background-light/90 dark:via-background-dark/90 to-transparent z-40">
        ...
        >
          ...
          <span className="text-[15px] uppercase tracking-widest">{t('bypass.activate_pro_sim')}</span>
