...
const MyNumbers: React.FC = () => {
    ...
    const handleSaveConfig = async () => {
        ...
        try {
            ...
            showToast("Error al guardar en Ledger", "error");
    ...
    const handleTestApi = async () => {
        ...
        setTestingApi(true);
        try {
            await fetch(apiUrl, { method: 'POST', mode: 'no-cors', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({telsim_test: true}) });
    ...
    const handleTestTg = async () => {
        ...
        try {
            ...
            await fetch(url, { 
                ...
                body: JSON.stringify({
                    chat_id: tgChatId,
                    text: "[TELSIM] ⚡ Test de conexión exitoso. Tu bot está listo para recibir SMS.",
                    parse_mode: 'Markdown'
                })
            });
                        ...
                        {slots.map((slot) => {
                            ...
                            return (
                                <div key={slot.port_id} className="relative group">
                                    <div className={`relative aspect-[1.58/1] w-full p-7 rounded-[2rem] flex flex-col justify-between shadow-2xl transition-all duration-500 overflow-hidden ${isPower ? 'bg-gradient-to-br from-[#B49248] to-[#8C6B1C]' : isPro ? 'bg-gradient-to-br from-[#0047FF] to-[#00E0FF]' : 'bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700'}`}>
                                        ...
                                        <div className="flex justify-between items-start z-10">
                                            <span className={`text-[12px] font-black tracking-tighter uppercase ${isPower || isPro ? 'text-white' : 'text-primary'}`}>Telsim Online</span>
                                        ...
                                        <div className="z-10">
                                            <span className={`text-[8px] font-black uppercase tracking-[0.3em] mb-1 block ${isPower || isPro ? 'text-white/40' : 'text-slate-400'}`}>Subscriber Number</span>
                                            <h3 className={`text-2xl font-black font-mono tracking-tighter ${isPower || isPro ? 'text-white' : 'text-slate-900 dark:text-white'}`}>{formatPhoneNumber(slot.phone_number)}</h3>
                                        </div>
                                        <div className="flex justify-between items-end z-10">
                                            <div className="px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest bg-white/20 backdrop-blur-md text-white flex items-center gap-1.5">
                                                {isPower ? <Crown className="size-3" /> : isPro ? <Zap className="size-3" /> : <Leaf className="size-3" />} {slot.actual_plan_name}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-5 flex items-center justify-center gap-3 px-1">
                                        ...
                                        <button onClick={() => { navigator.clipboard.writeText(slot.phone_number); showToast("Copiado"); }} className="size-12 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl flex items-center justify-center shadow-sm active:scale-95 transition-all">
