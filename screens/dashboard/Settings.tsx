...
const Settings: React.FC = () => {
  ...
  const Row = ({
    ...
    <div
      onClick={onClick}
      className="flex items-center gap-3 px-4 py-[13px] cursor-pointer active:bg-slate-50 dark:active:bg-slate-800 relative [&:not(:last-child)]:after:absolute [&:not(:last-child)]:after:bottom-0 [&:not(:last-child)]:after:left-[60px] [&:not(:last-child)]:after:right-4 [&:not(:last-child)]:after:h-px [&:not(:last-child)]:after:bg-slate-50 dark:[&:not(:last-child)]:after:bg-slate-800"
    >
      <div className="w-9 h-9 rounded-[10px] bg-[#eef2f7] dark:bg-slate-800 flex items-center justify-center flex-shrink-0 [&_svg]:stroke-[#1e3a8a] dark:[&_svg]:stroke-blue-400">
        {icon}
      </div>
  ...
  const Toggle = ({ enabled, onToggle }: { enabled: boolean; onToggle: () => void }) => (
    <div onClick={onToggle} className={`w-11 h-6 rounded-full relative cursor-pointer transition-colors duration-200 ${enabled ? 'bg-primary' : 'bg-slate-200 dark:bg-slate-700'}`}>
  ...
  return (
    <div className="min-h-screen bg-[#F0F4F8] dark:bg-background-dark font-display flex flex-col">
      ...
      <div className="bg-[#F0F4F8] dark:bg-background-dark pt-12 pb-3 px-5 flex items-center gap-3 flex-shrink-0">
        ...
        <h1 className="flex-1 text-[20px] font-black text-slate-900 dark:text-white tracking-tight">Ajustes</h1>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-28 space-y-3">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-100 dark:border-slate-800 flex items-center gap-3.5">
          <div className="relative flex-shrink-0 cursor-pointer" onClick={() => fileInputRef.current?.click()}>
            ...
            />
            {uploading ? (
              <div className="w-[62px] h-[62px] rounded-[18px] bg-slate-100 dark:bg-slate-800 border-[3px] border-white dark:border-slate-700 shadow-lg flex items-center justify-center">
              ...
              <img
                src={avatarUrl}
                alt={userName}
                className="w-[62px] h-[62px] rounded-[18px] object-cover border-[3px] border-white dark:border-slate-700 shadow-lg"
              />
            ) : (
              <div className="w-[62px] h-[62px] rounded-[18px] bg-gradient-to-br from-[#0ea5e9] to-primary flex items-center justify-center text-white text-[22px] font-black border-[3px] border-white dark:border-slate-700 shadow-lg">
                {userInitials}
              </div>
            )}
            <div className="absolute -bottom-1 -right-1 w-[22px] h-[22px] rounded-full bg-primary border-2 border-white dark:border-slate-700 flex items-center justify-center">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className="inline-flex items-center gap-1 bg-slate-100 dark:bg-slate-800 px-2 py-[3px] rounded-full text-[9.5px] font-semibold text-slate-500 dark:text-slate-400">
        ...
        <div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[18px] border border-slate-100 dark:border-slate-800 overflow-hidden">
            <Row icon={<svg width="17" height="17" viewBox="0 0 24 24" fill="none" strokeWidth="2.2" strokeLinecap="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/><circle cx="12" cy="16" r="1.5" fill={isDark ? "#60a5fa" : "#1e3a8a"} stroke="none"/></svg>} title="Seguridad y Contraseña" sub="Cambia tu contraseña" onClick={() => navigate('/dashboard/security')} right={<Chevron/>} />
        ...
        <p className="text-center text-[9px] font-bold text-slate-300 dark:text-slate-600 tracking-widest uppercase pb-2">Telsim v2.4.1</p>
      </div>
