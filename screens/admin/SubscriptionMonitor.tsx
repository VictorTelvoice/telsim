...
import { CreditCard, Loader2, ChevronUp, ChevronDown, Copy, X, Download } from 'lucide-react';
...
const SubscriptionMonitor: React.FC = () => {
    ...
    () =>
      subs.filter((s) => {
        if (dateFrom) {
          ...
          if (new Date(s.created_at).getTime() < from) return false;
        }
        if (dateTo) {
          const to = new Date(dateTo + 'T23:59:59.999').getTime();
          if (new Date(s.created_at).getTime() > to) return false;
        }
  ...
  // Contadores dinámicos: se recalculan al cambiar rango de fechas o buscador
                ...
                {displayedRows.map((s, idx) => (
                  ...
                  >
                    ...
                    <td
                      ...
                      onMouseEnter={() => setTooltipRowId(s.id)}
                    ...
                    >
                      <div className="flex items-center gap-1.5 min-w-0">
                        {s.user_id ? (
                          <>
                            ...
                            <button
                              type="button"
                              onClick={(e) => copyToClipboard(s.user_id, e)}
                      ...
                      {tooltipRowId === s.id && (
                        <div className="absolute left-4 top-full z-20 mt-1 px-2.5 py-1.5 rounded-lg bg-slate-800 text-white text-xs shadow-lg border border-slate-700 min-w-[180px]">
                          ...
                          <p className="text-white truncate">{s.user_nombre ?? '—'}</p>
                          <p className="font-semibold text-slate-200 mt-1">Email:</p>
                          <p className="text-white truncate">{s.user_email ?? '—'}</p>
                        </div>
                      )}
                    </td>
