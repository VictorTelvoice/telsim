...
const Success: React.FC = () => {
  ...
  // Si no hay datos en el estado de navegación, algo salió mal en el flujo
  if (!location.state) {
  ...
  const isError = assignedNumber === 'ERROR_ACTIVACION' || !assignedNumber;
        ...
        <div className={`relative z-10 w-full max-w-sm flex flex-col items-center text-center transition-all duration-700 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className="flex flex-col items-center mb-10">
                <div className="relative mb-6">
                    <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 rounded-full blur-2xl ${isError ? 'bg-rose-500/20' : 'bg-emerald-400/30'}`}></div>
                    <div className={`relative p-1 rounded-full shadow-sm transition-transform duration-500 ${showContent ? 'scale-100' : 'scale-0'}`}>
                        <div className={`p-2 rounded-full bg-white dark:bg-surface-dark`}>
                            <span className={`material-symbols-outlined text-[72px] leading-none block ${isError ? 'text-rose-500' : 'text-emerald-500'}`}>
                ...
                <div className="space-y-2 px-4">
                    <h1 className="text-3xl font-black text-[#111318] dark:text-white tracking-tight">
                        {isError ? 'Algo salió mal' : t('onboarding.success_title')}
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-[16px] leading-relaxed">
                        {isError 
                          ? 'No pudimos activar tu línea. Por favor contacta a soporte.' 
            ...
            {!isError && (
              <div className={`w-full rounded-[2.5rem] border-2 p-6 flex flex-col items-center space-y-4 mb-8 relative overflow-hidden transition-all duration-700 bg-white dark:bg-surface-dark border-primary/20 shadow-card`}>
                  
                  {/* Scanner Light Beam Animation */}
                  <div className={`absolute inset-0 w-[50%] h-[200%] -top-[50%] opacity-30 pointer-events-none animate-scanner bg-gradient-to-r from-transparent via-blue-400 to-transparent`}></div>
                  ...
                  <div className="flex flex-col items-center space-y-2 relative z-10 w-full">
                      <span className={`text-[11px] uppercase font-bold tracking-[0.2em] text-primary/70`}>
                          Puerto Privado Activado
                      </span>
                      
                      <div className="flex items-center gap-3 animate-reveal-number">
                          <div className={`size-8 rounded-full overflow-hidden border-2 shrink-0 border-slate-100 dark:border-slate-700`}>
                               <img 
                          ...
                          <div className={`font-mono text-[26px] font-black tabular-nums tracking-tighter text-[#111318] dark:text-white`}>
                  ...
                  <div className="flex items-center gap-2 relative z-10">
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border bg-emerald-50 dark:bg-emerald-500/10 border-emerald-100 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400`}>
                      ...
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border bg-slate-50 dark:bg-slate-700 border-slate-100 dark:border-slate-600 text-slate-600 dark:text-slate-300`}>
            ...
            <div className="w-full space-y-6">
                <button 
                    ...
                    className={`group relative w-full h-[64px] transition-all duration-300 rounded-2xl flex items-center justify-between pl-5 pr-2 shadow-xl active:scale-[0.98] bg-primary text-white hover:bg-blue-700 shadow-primary/20`}
                >
                    <div className="w-11"></div>
                    <span className="font-black text-[17px] tracking-wide text-center flex-1 uppercase">
                      {isError ? 'Volver al Inicio' : t('onboarding.enter_panel')}
                    </span>
                    <div className={`h-12 w-12 rounded-xl flex items-center justify-center transition-colors bg-white/10 group-hover:bg-white/20`}>
                        <span className={`material-icons-round text-[24px] text-white`}>arrow_forward</span>
                    </div>
                </button>
                ...
                {!isError && (
                  <div className="flex flex-col items-center gap-4">
                      <button 
                          onClick={handleCopy}
                          className={`group flex items-center justify-center gap-2 transition-colors w-full py-2 text-primary dark:text-blue-400 hover:text-blue-800`}
                      >
