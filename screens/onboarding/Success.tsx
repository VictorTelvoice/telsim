...
const Success: React.FC = () => {
  ...
  // PROTECCIÓN: Si no hay estado o hay flag de error, manejamos la redirección o el mensaje
  if (!location.state) {
    return <Navigate to="/dashboard" replace />;
  }

  const assignedNumber = location.state?.assignedNumber;
  const isError = location.state?.error || !assignedNumber || assignedNumber === 'ERROR_ACTIVACION';
        ...
        <div className={`relative z-10 w-full max-w-sm flex flex-col items-center text-center transition-all duration-700 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className="flex flex-col items-center mb-10">
                ...
                <div className="space-y-3 px-4">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-[15px] leading-relaxed max-w-[30ch] mx-auto">
                        {isError 
                          ? 'Hubo un problema al provisionar tu puerto físico. Nuestro equipo técnico ha sido notificado.' 
                          : 'Tu nuevo puerto físico ha sido sincronizado exitosamente con la red 4G LTE.'}
                    </p>
                </div>
            </div>

            {/* TARJETA DEL NÚMERO (Solo se muestra si NO hay error) */}
            {!isError && assignedNumber && (
              <div className={`w-full rounded-[2.5rem] border-2 p-7 flex flex-col items-center space-y-5 mb-10 relative overflow-hidden transition-all duration-700 bg-white dark:bg-surface-dark border-primary/20 shadow-card group`}>
                  ...
                  <div className="flex flex-col items-center space-y-3 relative z-10 w-full">
                      <span className={`text-[11px] uppercase font-black tracking-[0.25em] text-primary/70`}>
                          PUERTO ASIGNADO
                      </span>
            ...
            <div className="w-full space-y-6">
                ...
                >
                    ...
                    <span className="font-black text-[15px] tracking-[0.1em] text-center flex-1">
                ...
                {!isError && (
                  <div className="flex flex-col items-center gap-5">
                      ...
                      <div className="flex items-center gap-3 opacity-30">
                          ...
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em]">Hardware TELSIM v4.5</span>
                          <div className="h-px w-10 bg-slate-400"></div>
                      </div>
                  </div>
                )}
