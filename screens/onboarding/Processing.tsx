  ...
  useEffect(() => {
    const processSubscription = async () => {
      ...
      try {
        ...
        // 1. LLAMADA ÚNICA AL RPC (Sin .headers())
        ...
        // 2. MANEJO DE ERROR 23505 (DUPLICADO)
        if (rpcError && rpcError.code === '23505') {
          // Ignoramos el error y buscamos la suscripción existente
          ...
          // Si es otro tipo de error, salimos al dashboard
          throw rpcError;
        }

        // 3. NAVEGACIÓN DE ÉXITO O FALLBACK
        if (finalNumber) {
  ...
  return (
    <div className="flex h-screen w-full flex-col items-center justify-center bg-background-light dark:bg-background-dark font-display">
      <div className="flex flex-col items-center gap-6 animate-in fade-in duration-700">
        ...
        <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.3em] italic">Preparando tu línea...</p>
      </div>
      
      <div className="absolute bottom-12 opacity-20">
        <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.5em]">TELSIM PURE-SYNC v3.0</p>
      </div>
    </div>
  );
};
