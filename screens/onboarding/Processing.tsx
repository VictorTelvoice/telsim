...
const Processing: React.FC = () => {
  ...
  useEffect(() => {
    const startProvisioning = async () => {
      // Protección contra múltiples clics o ejecuciones concurrentes
      ...
      try {
        ...
        // LLAMADA RPC ESTRICTA AL CORE DE TELSIM
        ...
        if (rpcError) {
          isSubmitting.current = false; // Liberar bloqueo en caso de error para permitir reintento
          throw rpcError;
        }

        // Extracción robusta del número (soporta snake_case y camelCase de la respuesta)
        const finalNumber = rpcResult?.phoneNumber || rpcResult?.phone_number;

        if (!finalNumber) {
          // Si no hay número pero la función no dio error, el sistema Success.tsx intentará el rescate
          navigate(`/onboarding/success?planName=${encodeURIComponent(planName)}`, { replace: true });
          return;
        }

        setProgress(100);
        setCurrentStep('Línea activada correctamente');

        // Registro en el Centro de Notificaciones
        addNotification({
          title: 'SIM Activada',
          message: `Línea ${finalNumber} aprovisionada correctamente bajo el plan ${planName}.`,
          type: 'activation',
          details: {
            number: finalNumber,
            plan: planName,
        ...
        // Delay estético para confirmación visual del 100%
        ...
        console.error("Fallo crítico de aprovisionamiento:", err);
        setErrorState(err.message || "Error de conexión con la infraestructura de red.");
    ...
    <div className="relative flex h-screen w-full flex-col items-center justify-center overflow-hidden bg-background-light dark:bg-background-dark font-display">
      ...
      <div className="absolute bottom-12 w-full text-center opacity-30">
        <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.5em]">
          TELSIM CORE INFRASTRUCTURE v7.0
        </p>
      </div>
    </div>
  );
