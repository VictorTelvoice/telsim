...
import { Loader2, AlertCircle, Cpu, Globe, RefreshCw } from 'lucide-react';
...
const Processing: React.FC = () => {
  ...
  const maxPolling = 40; // Reducido a ~100 segundos para no agotar al usuario
  ...
  const checkSubscription = async () => {
    ...
    try {
      // 1. INTENTO PRIMARIO: Por Session ID (Lo más preciso)
      ...
      // 2. INTENTO SECUNDARIO (RESCATE): Si no hay session_id pero el usuario ya tiene una línea activa reciente
      if (!data && user) {
          ...
          // Si encontramos una suscripción activa creada en los últimos 5 minutos, la damos por válida
          if (recentSub) {
              const createdDate = new Date(recentSub.created_at).getTime();
              const now = new Date().getTime();
              if (now - createdDate < 300000) { // 5 minutos
                  data = recentSub;
              }
          }
      }
  ...
  const handleManualVerify = async () => {
    ...
    try {
      // Forzamos búsqueda de emergencia directamente en la tabla de slots si la suscripción falla
      ...
      if (slotData?.phone_number) {
        navigate(`/onboarding/success?session_id=${sessionId}&planName=${encodeURIComponent(planName)}&assignedNumber=${encodeURIComponent(slotData.phone_number)}`, { replace: true });
      } else {
        alert("Aún no detectamos tu puerto físico asignado. Por favor, contacta a soporte si el problema persiste.");
      }
    } catch (err) {
      setError("RETRY_DB");
    } finally {
  ...
  return (
    <div className="flex h-screen w-full flex-col items-center justify-center bg-background-light dark:bg-background-dark font-display p-8 overflow-hidden text-center">
      ...
      <div className="flex flex-col items-center gap-12 w-full max-w-sm relative z-10">
        {error === "TIMEOUT" ? (
          <div className="w-full flex flex-col items-center space-y-8 animate-in zoom-in duration-300">
            <div className="size-20 bg-amber-500/10 rounded-[2.5rem] flex items-center justify-center border border-amber-500/20 shadow-lg">
            ...
            <div className="w-full space-y-3">
                ...
                <button onClick={() => navigate('/dashboard')} className="w-full h-12 text-slate-400 font-black uppercase text-[10px] tracking-widest">Omitir e ir al Panel</button>
            </div>
          </div>
        ) : (
          <>
      ...
      <div className="absolute bottom-12 flex items-center gap-4 opacity-20">
        ...
        <span className="text-[8px] font-black uppercase tracking-[0.5em]">TELSIM CLOUD v4.2</span>
      </div>
    </div>
  );
};
