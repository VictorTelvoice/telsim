...
import { submitContactForm } from '../../actions/publicActions';
...
const Landing: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleCasos = () => {
      if (!casosEl) return;
      setCasosPage(Math.round(casosEl.scrollLeft / (casosEl.scrollWidth / (casosUso ? casosUso.length : 1))));
    };
    const handlePlanes = () => {
      if (!planesEl) return;
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    if (!contactForm.name || !contactForm.email || !contactForm.message) {
      setContactFeedback({ type: 'error', message: 'Por favor, completa todos los campos requeridos.' });
      return;
    }
    ...
    try {
      const res = await submitContactForm({ ...contactForm, language });
      if (res.success) {
        setContactFeedback({ type: 'success', message: '¡Mensaje enviado con éxito! Nos contactaremos pronto.' });
        setContactForm({ name: '', company: '', email: '', message: '', language });
      } else {
        setContactFeedback({ type: 'error', message: res.error || 'Error al enviar el mensaje.' });
      }
    } catch {
      setContactFeedback({ type: 'error', message: 'Ocurrió un error inesperado al enviar el formulario.' });
    } finally {
        ...
        <div className="max-w-5xl mx-auto h-16 px-3 sm:px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button
              ...
              className="flex items-center gap-1 px-2 py-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-[11px] font-black text-slate-500 dark:text-slate-400 border border-slate-100 dark:border-slate-800"
            ...
            <button onClick={() => navigate('/login')} className="text-sm font-bold text-slate-600 dark:text-slate-300 hover:text-primary transition-colors">{t('landing.nav.login')}</button>
            ...
            >
              {[
                ...
                <div key={i} className="flex flex-col items-center gap-3 group">
                  <img
                    src={brand.logo ? brand.logo : `https://cdn.simpleicons.org/${brand.slug}/white`}
                    alt={brand.name}
