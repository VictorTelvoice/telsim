...
const Summary: React.FC = () => {
  ...
  useEffect(() => {
    if (!user?.id) return;
    updateOnboardingStep(ONBOARDING_STEPS.SUMMARY);
  }, [user?.id]);

  const planData = useMemo(() => {
    if (typeof window === 'undefined') return {};

    // Reconstruct from localStorage so back-navigation works in Next.js
  ...
  const planName = (planData as any).planName || 'Pro';
  const isAnnual = (planData as any).isAnnual || false;
  const stripePriceId = (planData as any).stripePriceId || (PLAN_CATALOGUE[planName] ? PLAN_CATALOGUE[planName].monthlyId : '');
  ...
  const handleNext = () => {
    ...
    // In Next.js we use localStorage instead of route state for cross-page persistence in onboarding
    ...
    return (
      <div className="min-h-screen bg-[#F0F4F8] dark:bg-background-dark font-display flex flex-col">
        ...
        <header className="bg-white dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800 px-8 py-4 flex items-center justify-between">
          ...
          <button
            onClick={() => !isNavigating && router.back()}
    ...
    <div className="bg-background-light dark:bg-background-dark font-display text-[#111318] dark:text-white antialiased min-h-screen flex flex-col items-center">
      <div className="relative flex h-full min-h-screen w-full max-w-md flex-col bg-background-light dark:bg-background-dark overflow-x-hidden shadow-2xl">
        <div className="sticky top-0 z-20 flex items-center bg-background-light/90 dark:bg-background-dark/90 px-4 py-3 backdrop-blur-sm">
          <div onClick={() => !isNavigating && router.back()} className={`flex size-10 shrink-0 items-center justify-center rounded-full hover:bg-black/5 dark:hover:bg-white/10 cursor-pointer ${isNavigating ? 'opacity-30' : ''}`}>
