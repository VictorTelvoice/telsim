...
function getPlanName(): string {
  ...
  const raw = typeof window !== 'undefined' ? localStorage.getItem('selected_plan') || 'pro' : 'pro';
...
const RegionSelect: React.FC = () => {
  ...
  useEffect(() => {
    if (!user?.id) return;
    updateOnboardingStep(ONBOARDING_STEPS.REGION);
  }, [user?.id]);
  ...
  const handleContinue = () => {
    // Persistimos para que Summary y Payment puedan reconstruir el estado.
    if (typeof window !== 'undefined') {
      ...
      // Actualizamos localStorage con los valores resueltos para Summary.tsx
      localStorage.setItem('selected_plan_name', resolvedPlanName);
      localStorage.setItem('selected_plan_price', String(resolvedPrice));
      localStorage.setItem('selected_plan_price_id', resolvedPriceId);
      localStorage.setItem('selected_plan_limit', String(resolvedLimit));
    }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background-light dark:bg-background-dark text-slate-800 dark:text-slate-100 p-6 pt-8 font-display">
      <div className="flex justify-between items-center mb-12">
        <button onClick={() => router.push('/onboarding/plan')} className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 transition-colors">
