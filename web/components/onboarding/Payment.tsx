...
import { updateOnboardingStep, updateOnboardingCheckoutSession } from '@/actions/onboardingActions';
...
const Payment: React.FC = () => {
  ...
  useEffect(() => {
    if (!user?.id) return;
    updateOnboardingStep(ONBOARDING_STEPS.PAYMENT);
  }, [user?.id]);

  // ─── Fallback: read from localStorage as Next.js lacks cross-page 'state' in router.push
  const _lsId = typeof window !== 'undefined' ? localStorage.getItem('selected_plan') : null;
  const _lsAnnual = typeof window !== 'undefined' ? localStorage.getItem('selected_plan_annual') === 'true' : false;
  const _lsPrice = typeof window !== 'undefined' ? parseFloat(localStorage.getItem('selected_plan_price') || '0') || 0 : 0;
  const _lsPriceId = typeof window !== 'undefined' ? localStorage.getItem('selected_plan_price_id') || '' : '';
  const _nameMap: Record<string, string> = { starter: 'Starter', pro: 'Pro', power: 'Power' };

  const planName = (_lsId ? _nameMap[_lsId] : null) || 'Starter';
  const isAnnual = _lsAnnual;
  const price = _lsPrice || 19.90;
  const monthlyLimit = (({ Starter: 150, Pro: 400, Power: 1400 } as Record<string, number>)[planName] ?? 150);
  const stripePriceId = _lsPriceId || '';
  const region = (typeof window !== 'undefined' ? localStorage.getItem('selected_region') || undefined : undefined);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const checkoutSessionId = typeof data.checkoutSessionId === 'string' ? data.checkoutSessionId : null;
      if (checkoutSessionId) {
        await updateOnboardingCheckoutSession(checkoutSessionId);
      }
