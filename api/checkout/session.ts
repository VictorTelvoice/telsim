...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // 2. LÓGICA DE PAGO INSTANTÁNEO (NUEVA O UPGRADE)
    if (customerId && !forceManual) {
      ...
      if (!isUpgrade) {
        try {
          // Buscamos si el cliente tiene un método de pago por defecto en Stripe
          ...
          if (defaultPaymentMethod) {
            ...
            // Reservamos el slot temporalmente para evitar race conditions
            await supabaseAdmin.from('slots').update({ 
                status: 'reservado', 
                assigned_to: userId 
            }).eq('slot_id', freeSlot.slot_id);

            // Creamos la suscripción directamente
            ...
            // IMPORTANTE: En modo instantáneo, provisionamos aquí mismo ya que no hay redirect de Checkout
            ...
            return res.status(200).json({ 
                instant: true, 
                phoneNumber: freeSlot.phone_number,
                message: 'Nueva línea activada instantáneamente.' 
            });
          }
        } catch (instantErr: any) {
          console.error("[INSTANT NEW SUB FAIL]", instantErr.message);
          // Si falla, permitimos que el código siga hacia el flujo manual de Checkout
        }
      }
    }

    // 3. FLUJO ESTÁNDAR DE CHECKOUT (PARA NUEVOS O FALLOS EN INSTANTÁNEO)
